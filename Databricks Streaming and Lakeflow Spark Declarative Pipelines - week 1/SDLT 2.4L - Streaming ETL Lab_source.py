...
# MAGIC
# MAGIC This notebook programmatically generates and triggers an update of a Lakeflow Spark Declarative pipeline that consists of the following notebooks:
# MAGIC
...
# MAGIC
# MAGIC **NOTE:** Re-running the entire notebook will delete the underlying data files for both the source data and your Lakeflow Spark Declarative pipeline.
...
# MAGIC %md
# MAGIC
# MAGIC ## Generate the Lakeflow Spark Declarative pipeline
...
spark.sql("DROP TABLE IF EXISTS dbacademy.labuser13429858_1768515742.bpm_bronze")
spark.sql("DROP TABLE IF EXISTS dbacademy.labuser13429858_1768515742.bpm_quarantine")
spark.sql("DROP TABLE IF EXISTS dbacademy.labuser13429858_1768515742.bpm_silver")
spark.sql("DROP TABLE IF EXISTS dbacademy.labuser13429858_1768515742.bronze")
spark.sql("DROP TABLE IF EXISTS dbacademy.labuser13429858_1768515742.distinct_topics")
...
# MAGIC %md
# MAGIC #### REQUIRED!!!: Make sure to complete the lab in the Lakeflow Spark Declarative pipeline notebook linked above ([Streaming ETL Lab]($./Pipeline/SDLT 2.4.1L - Streaming ETL Lab)) before running this pipeline.
# MAGIC
# MAGIC After editing the final notebook, run the cell below to start the pipeline.
# MAGIC
# MAGIC **NOTE:** If prompted to enable the new Lakeflow Spark Declarative pipeline editor, you can choose to enable it if you're already familiar with it or if you'd like to explore the new interface.
# MAGIC
# MAGIC **📌 NOTE: The pipeline may fail because the catalog used in previous pipelines is still in use. If this happens, please wait for 1–2 minutes and then rerun the pipeline.**
